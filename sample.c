...
// Don't do this ->    MEM_WRITE( Z502DiskSetBuffer, (INT32 *)disk_buffer_write );
    ...
    *********************************************************************/
    ...
    printf( "2.  Start State = locked(1): Action (Thread 1) = unLock: End State = UnLocked\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_UNLOCK, SUSPEND_UNTIL_LOCKED, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );

    printf( "3.  Start State = Unlocked:  Action (Thread 1) = unLock: End State = UnLocked\n");
    printf( "    An Error is Expected\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_UNLOCK, SUSPEND_UNTIL_LOCKED, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );

    printf( "4.  Start State = unlocked:  Action (Thread 1) = tryLock: End State = Locked\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_LOCK, DO_NOT_SUSPEND, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );

    printf( "5.  Start State = Locked(1): Action (Thread 1) = tryLock: End State = Locked\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_LOCK, DO_NOT_SUSPEND, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );

    printf( "6.  Start State = locked(1): Action (Thread 1) = unLock: End State = UnLocked\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_UNLOCK, SUSPEND_UNTIL_LOCKED, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );

    printf( "7.  Start State = Unlocked:  Action (Thread 1) = Lock: End State = Locked\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_LOCK, SUSPEND_UNTIL_LOCKED, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );

    //  A thread that locks an item it has already locked will succeed
    printf( "8.  Start State = locked(1): Action (Thread 1) = Lock: End State = Locked\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_LOCK, SUSPEND_UNTIL_LOCKED, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );

    // Locking a thread that's already locked - but do it with a thread
    // other than the one that did the locking.
    printf( "9.  Start State = Locked(1): Action (Thread 2) = tryLock: End State = Locked\n");
    printf( "    An Error is Expected\n");
    ...
    printf( "11. Start State = Locked(1): Action (Thread 2) = Lock: End State = Locked\n");
    ...
    printf( "12. Start State = locked(1): Action (Thread 1) = unLock: End State = Locked(by 2)\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_UNLOCK, SUSPEND_UNTIL_LOCKED, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );
    DoSleep(100);            /*  Wait for locking action of 2nd thread to finish   */

    printf( "13. Start State = Locked(2): Action (Thread 3) = unLock: End State = Locked(2)\n");
    printf( "    An Error is Expected\n");
    Status = CreateAThread( (int *)DoOneUnlock, &Temp);
    DoSleep(100);            /*  Wait for that thread to finish   */

    printf( "14. Start State = Locked(2): Action (Thread 1) = tryLock: End State = Locked(2)\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_LOCK, DO_NOT_SUSPEND, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );

    printf( "15. Start State = locked(2): Action (Thread 1) = unLock: End State = Locked(2)\n");
    printf( "    An Error is Expected\n");
    Z502_READ_MODIFY( MEMORY_INTERLOCK_BASE, DO_UNLOCK, SUSPEND_UNTIL_LOCKED, &LockResult );
    printf( "%s\n", &(Success[ SPART * LockResult ]) );
